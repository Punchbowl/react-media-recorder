...
export const ReactMediaRecorder = ({
  ...
  useEffect(() => {
    ...
    if (mediaRecorderOptions && mediaRecorderOptions.mimeType) {
      if (!MediaRecorder.isTypeSupported(mediaRecorderOptions.mimeType)) {
        throw new Error("The specified MIME type you supplied for MediaRecorder doesn't support this browser")
      }
    }
  ...
  const onRecordingStop = () => {
    const blobProperty: BlobPropertyBag =
      blobPropertyBag || video ? { type: mediaRecorderOptions && mediaRecorderOptions.mimeType || 'video/mp4' } : { type: 'audio/wav' }
